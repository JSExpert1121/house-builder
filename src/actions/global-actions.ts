...
export const getProposalData = id => dispatch => {
	...
	return Axios.get(process.env.REACT_APP_PROJECT_API + 'proposals/' + id).then(res => {
...
export const addFilesToProposal = (id, files) => () => PropApi.addFiles(id, files);
export const deleteProposalFile = (id, name) => () => PropApi.deleteFile(id, name);
