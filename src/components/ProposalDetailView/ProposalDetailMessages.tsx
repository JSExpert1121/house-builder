...
interface ProposalDetailMessagesProps {
	proposal: any,
	getProposalMessages: (id: number, something: number, pageSize: number, cb: any) => any,
	addMessageToProposal: (prop_id: number, message: any, cb: any, cont_type: any) => any,
		...
		for (let i = 0; i < messageList.length; i++) {
			...
			for (let j = 0; j < message.proposalMessageFiles.length; j++) {
				...
				msgFiles.push(<h5 key={j}><a download={row.name}
				                             href={process.env.PROJECT_API + "/messages/" + message.id + "/files/" + row.name}>{row.name}</a>
				</h5>);
			}
					...
					<div className={classes.listRoot}>
						{
							...
							>
								...
								<CircularProgress key={'loader'} className={classes.waitingSpin} size={24} thickness={4}/>}
								{renderMessages}
