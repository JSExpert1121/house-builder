...
const ProposalView = ({ classes, proposal }) => {
    return (
        <Box className={classes.root}>
            <Typography variant="subtitle1" noWrap style={{ fontWeight: '600', fontSize: '20px', marginTop: '0' }}>Proposal</Typography>
                    ...
                    <Box className={classes.brief}>
                        {proposal.proposalFiles && proposal.proposalFiles.length > 0 && (
                            <>
                                ...
                                {proposal.proposalFiles.map(file => <Typography className={classes.desc} key={file.id}>{file.name}</Typography>)}
                            </>
                        )}
                    </Box>
                </Grid>
