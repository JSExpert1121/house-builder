...
class ConnectedSubmittedProView extends React.Component {
	...
	componentDidMount() {
		...
		this.props.getProposals(userProfile.user_metadata.contractor_id, 0, 0, 'AWARDED');
	}
	...
	handleChangePage = (event, page) => {
		...
		this.props.getProposals(userProfile.user_metadata.contractor_id, page, this.state.rowsPerPage, 'AWARDED');
	};
	...
	handleChangeRowsPerPage = event => {
		...
		this.props.getProposals(userProfile.user_metadata.contractor_id, currentPage, rowsPerPage, 'AWARDED');
	};
		...
		await this.props.deleteProposal(id, (res) => {
			...
			if (res) {
				...
				if (this.state.rowsPerPage * (this.state.currentPage) < proposals.totalElements - 1) {
					this.props.getProposals(userProfile.user_metadata.contractor_id, this.state.currentPage, this.state.rowsPerPage, 'AWARDED');
				}
				else {
					...
					this.props.getProposals(userProfile.user_metadata.contractor_id, currentPage, this.state.rowsPerPage, 'AWARDED');
				}
			}
		})
	}
